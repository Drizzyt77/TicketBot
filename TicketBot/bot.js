...
client.on('interactionCreate', async interaction => {
	async function openTicket(user, type) {
		console.log(`${type} ticket opened by ${user.name}`)
		...
		if (sqldb === 'pvekaldiscord') {
			kalpve.query("SELECT SteamId FROM discordsteamlinks WHERE DiscordId = ?", [user.id], function (err, player) {
				...
				if (!player.length) {
					...
					kalpve.query("UPDATE open SET linked = ? WHERE channel = ?", [true, channel.id], function (err, results) {
						if (err) { }
					});
			...
			kalprimal.query("SELECT SteamId FROM discordsteamlinks WHERE DiscordId = ?", [user.id], function (err, player) {
				...
				if (!player.length) {
					...
					kalprimal.query("UPDATE open SET linked = ? WHERE channel = ?", [true, channel.id], function (err, results) {
						if (err) { }
					});
					sendTicketInfo(user, channel, Object.values(player[0])[0]);
				}
			...
			kalomega.query("SELECT SteamId FROM discordsteamlinks WHERE DiscordId = ?", [user.id], function (err, player) {
				...
				if (!player.length) {
					...
					kalomega.query("UPDATE open SET linked = ? WHERE channel = ?", [true, channel.id], function (err, results) {
						if (err) { }
					});
					sendTicketInfo(user, channel, Object.values(player[0])[0]);
				}
	...
	function cancelTicket(channel) {
		console.log(`${channel.name} ticket closed by ${interaction.member.displayName}`)
		channel.delete();
    }
